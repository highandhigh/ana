...
sell = function(i, count){
  if(count >= 351 && share > 0 && single[i,"Close"]  < single[i,closeStr] - 2.5* single[i,sdStr]){
    profit = 0
    while(share > 0){
...
buy = function(i,count){
  if(count >= 351 && single[i,"Close"] > single[i,closeStr] + 2.5 * single[i,sdStr]){
    if(share >= 4){
