...
low = 5
high = 50
highStr = paste("AvgHigh",high,sep="")
lowStr = paste("AvgLow",low,sep="")
...
calc = function(i,count){
  ...
  calAvgLow(i,count,low)
  calAvgHigh(i,count,high)
}

sell = function(i, count){
  if(count >= 50 && share > 0 && 
     (single[i,highStr]  > single[i,lowStr])){
    profit = 0
    while(share > 0){
...
buy = function(i,count){
  if(count >= 50 && single[i,highStr] < single[i,lowStr]){
    if(share >= 4){
